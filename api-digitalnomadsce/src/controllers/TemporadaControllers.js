...
class TemporadaController {
  ...
  static async cadastraTemporada(req, res) {
    ...
    try {
      const criarTemporada = await database.TemporadaNomad.create(novaTemporada);
  ...
  static async pegaTemporadas(req, res) {
    try {
      ...
      const mostraTemporada = await database.TemporadaNomad.findAll({
        ...
        attributtes: ["nome_evento", "data_evento", "horario", "tipo_evento", "local", "endereco" ],
         include: [
          {
  ...
  static async anexoImagem(req, res) {
      ...
      if(type == "image/jpeg" || type == "image/png" || type == "image/jpg"){
        try {      
          ...
          return res.status(200).json({message: 'Banner de divulgação anexado com sucesso!'});
        } catch (error) {
          return res.status(500).json(error.message);
        }
      } else {
        return res.status(500).json({
          message: "Somente arquivo com extensão .jpg, .jpeg ou .png são permitidos!",
        });
      }
