...
export class ServiceService {
  ...
  cadastrar_parceiro(data: FormData): Observable<any> {
  ...
  updateNomad(data: any, id: any) {
    return this.http.put<any>(environment.url + 'nomads/' + id, data).pipe(
      map((res: any) => {
        return res;
      }),
    );
  ...
  updateCompany(data: any, id: any) {
    return this.http
      .put<any>(environment.url + 'companynomads/' + id, data)
      .pipe(
        map((res: any) => {
          return res;
        }),
  ...
  consultaIBGE(ibge: string) {
    return this.http.get(
      `https://servicodados.ibge.gov.br/api/v3/agregados/6579/periodos/2021/variaveis/9324?localidades=N6[${ibge}]`,
    );
  }
}
